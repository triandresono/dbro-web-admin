...
class SysconfigRepositoryImpl extends SysconfigRepository {
  ...
  @override
  Future<Either<StatusResponse, RoleDropdown>> get roleDropdown async {
    try {
      final result = await datasource.roleDropdown;
      return result.fold(
        (failure) => Left(failure),
        (result) => Right(RoleDropdown.fromMap(result.toMap())),
      );
    } catch (e) {
      if (e is Map) {
        return Left(StatusResponse.failure(e as Map<String, dynamic>));
